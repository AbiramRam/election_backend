package org.election.handler;

import com.sun.net.httpserver.HttpHandler;
import com.sun.net.httpserver.HttpExchange;
import java.io.IOException;
import java.io.OutputStream;
import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;

import org.election.config.DBConnection;
import org.json.JSONArray;
import org.json.JSONObject;
import java.util.Base64;

public class GetPartiesHandler implements HttpHandler {
    @Override
    public void handle(HttpExchange exchange) throws IOException {
        if ("GET".equals(exchange.getRequestMethod())) {
            try (Connection conn = DBConnection.getConnection()) {
                String sql = "SELECT * FROM party ";
                PreparedStatement pstmt = conn.prepareStatement(sql);
                ResultSet rs = pstmt.executeQuery();

                JSONArray parties = new JSONArray();
                while (rs.next()) {
                    JSONObject party = new JSONObject();
                    party.put("id", rs.getInt("id"));
                    party.put("partyName", rs.getString("partyName"));
                    party.put("idElection", rs.getInt("idElection"));


                    byte[] logoBytes = rs.getBytes("partyLogo");
                    if (logoBytes != null) {
                        String logoBase64 = Base64.getEncoder().encodeToString(logoBytes);
                        party.put("partyLogo", logoBase64);
                    } else {
                        party.put("partyLogo", JSONObject.NULL);
                    }

                    parties.put(party);
                }

                JSONObject response = new JSONObject();
                response.put("status", "success");
                response.put("parties", parties);
                sendResponse(exchange, 200, response.toString());
            } catch (Exception e) {
                sendResponse(exchange, 500, "{\"status\":\"error\",\"message\":\"" + e.getMessage() + "\"}");
            }
        } else {
            sendResponse(exchange, 405, "{\"status\":\"error\",\"message\":\"Method not allowed\"}");
        }
    }

    private void sendResponse(HttpExchange exchange, int statusCode, String response) throws IOException {
        exchange.getResponseHeaders().set("Content-Type", "application/json");
        exchange.sendResponseHeaders(statusCode, response.getBytes().length);
        OutputStream os = exchange.getResponseBody();
        os.write(response.getBytes());
        os.close();
    }
}